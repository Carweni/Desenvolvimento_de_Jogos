...
public class GameScreen implements Screen {
    ...
    public GameScreen(MinerShip m) {
        ...
        // tiled map
        map = new TmxMapLoader().load("tiledMap/mapa1.tmx");
        ...
        for(int i=0;i<19;i++) {
            for(int j=0;j<12;j++) {
                ...
                if(r.nextFloat() < 0.8f) {
                    if(result<=20){
                        t = WallWorld.assetManager.get("minerioBlock2.png");
                        tipo = "ouro";
                        tempo=2;
                    }else if(result<=30){
                        t = WallWorld.assetManager.get("minerioBlock3.png");
                        tipo = "minerio";
                        tempo=3;
                    } else{
                        t = WallWorld.assetManager.get("block.png");
                        tipo = "bloco";
                        tempo = 1;
                    }
                    ...
                    if(j<=1 && (i>1 && i<5)){
                        ...
                        Blocks.add(aux);
                    } else if((j<=3 && j>=2) && (i>0 && i<8)){
                        Block aux = new Block(504+(72*(i)), (j*72)+216, t, tipo, tempo, new ParticleEffect(blockBreakEffect));
                        Blocks.add(aux);
                    } else if((j<=6 && j>=4) && (i>1 && i<11)){
                        Block aux = new Block(504+(72*(i)), (j*72)+216, t, tipo, tempo, new ParticleEffect(blockBreakEffect));
                        Blocks.add(aux);
                    } else if(j==7 && (i>3 && i<11)){
                        Block aux = new Block(504+(72*(i)), (j*72)+216, t, tipo, tempo, new ParticleEffect(blockBreakEffect));
                        Blocks.add(aux);
                    } else if((j<=10 && j>=8) && (i>6 && i<15)){
                        Block aux = new Block(504+(72*(i)), (j*72)+216, t, tipo, tempo, new ParticleEffect(blockBreakEffect));
                        Blocks.add(aux);
                    } else if(j==11 && (i>10 && i<14)){
                        Block aux = new Block(504+(72*(i)), (j*72)+216, t, tipo, tempo, new ParticleEffect(blockBreakEffect));
                        Blocks.add(aux);
                    }

                    if((j==5||j==8) && (i>14 && i<19)){
                        Block aux = new Block(504+(72*(i)), (j*72)+216, t, tipo, tempo, new ParticleEffect(blockBreakEffect));
                        Blocks.add(aux);
                    }
                    if((j==6 || j==7) && (i>12 && i<19)){
                        Block aux = new Block(504+(72*(i)), (j*72)+216, t, tipo, tempo, new ParticleEffect(blockBreakEffect));
                        Blocks.add(aux);
                    }
                }
    ...
    public void render(float delta) {
        ...
        Texture backgroundTexture = WallWorld.assetManager.get("background.jpg", Texture.class);
        ...
        for (Block aux : Blocks) {
            ...
            if(!aux.isActive() && aux.isInOreState()){
                if(aux.getTipo().equals("ouro")){
                    aux.makeOre(WallWorld.assetManager.get("honey_small.png"));
                    batch.draw(aux.getTexture(), aux.getX()+(aux.getWidth()/2)-10, aux.getY()+(aux.getHeight()/2)-10, 20, 20);
                } else if(aux.getTipo().equals("minerio")){
                    aux.makeOre(WallWorld.assetManager.get("coil_small.png"));
                    batch.draw(aux.getTexture(), aux.getX()+(aux.getWidth()/2)-10, aux.getY()+(aux.getHeight()/2)-10, 20, 20);
                }
            }
        }
        ...
        batch.draw((Texture) WallWorld.assetManager.get("honey_small.png", Texture.class),
                countPositionX + 50, WORLD_HEIGHT-patch.getTotalHeight()-20, 25, 25);
        font.draw(batch, "- " + minerShip.getOreCount()[0], countPositionX + 50+30, WORLD_HEIGHT-75);
        batch.draw((Texture) WallWorld.assetManager.get("coil_small.png", Texture.class),
                countPositionX + 50, WORLD_HEIGHT-patch.getTotalHeight()-50, 25, 25);
        font.draw(batch, "- " + minerShip.getOreCount()[1], countPositionX + 50+30, WORLD_HEIGHT-105);
        batch.draw((Texture) WallWorld.assetManager.get("diamond-small.png", Texture.class),
                countPositionX + 50, WORLD_HEIGHT-patch.getTotalHeight()-80, 25, 25);
        font.draw(batch, "- " + minerShip.getOreCount()[2], countPositionX + 50+30, WORLD_HEIGHT-135);
        batch.draw((Texture) WallWorld.assetManager.get("chalcopyrite-small.png", Texture.class),
                countPositionX + 50, WORLD_HEIGHT-patch.getTotalHeight()-110, 25, 25);
        ...
        if (currentState == GameState.PLAYING) {
            ...
            GameScreen2 newGameScreen2 = new GameScreen2(minerShip);
            ((WallWorld) Gdx.app.getApplicationListener()).setScreen(newGameScreen2);
        }
